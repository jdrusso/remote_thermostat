...
FORMAT = "%(message)s"
logging.basicConfig(
    format=FORMAT, level=logging.DEBUG, datefmt="[%x %X]", handlers=[RichHandler(), RichHandler(console=logfile_console, omit_repeated_times=False)]
)
...
class Thermostat:
    ...
    def update_state(self, current_temp):
        ...
        # In hysteresis
            ...
            status = (self.heating<<1) + self.cooling
            status_str = ['off', 'cool', 'heat', 'both???']
            self.log.critical(f"Off, maintaining {status_str[status]}")
            pass
            #print("Turning off")
            #self.all_off()
...
if __name__=="__main__":
    ...
    thermostat.schedule.add_range([69, 74, 1.5], [datetime.time(8, 0), datetime.time(12, 0)])
    
    # From 12PM to 11PM, set the temp to 71-73 with 1.5 degree hysteresis
    thermostat.schedule.add_range([72, 74, 0.50], [datetime.time(10, 0), datetime.time(23, 0)])

    # From 11PM to 8AM, set the temp to 62-69 with 1.5 degree hysteresis
    thermostat.schedule.add_range([62, 74, 1.], [datetime.time(23, 0), datetime.time(8, 0)])
    ...
    while True:
        ...
        if 300 > recv_temp > -100:
            ...
            cur_temp = calibrate_temp(raw_cur_temp, *remote_thermometer_calibration)
            log.info(f"Remote| Current: {raw_cur_temp:.2f}. Calibrated: {cur_temp:.2f}")
